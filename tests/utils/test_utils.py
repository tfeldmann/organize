...
@pytest.mark.parametrize(
    "filesystem,path,args,result_fs,result_path",
    [
...
def test_fs_path_expand(filesystem, path, args, result_fs, result_path):
    assert fs_path_expand(
        filesystem=filesystem,
        path=path,
        working_dir=".",
        args=args,
    ) == (result_fs, result_path)
